...
struct _GtkCifroCurveRendererPrivate
{
  ...
  gboolean             visible_update;     /* Признак необходимости перерисовки кривой. */
...
static void            gtk_cifro_curve_renderer_set_property           (GObject                       *object,
                                                                        guint                          prop_id,
                                                                        const GValue                  *value,
                                                                        GParamSpec                    *pspec);
static void            gtk_cifro_curve_renderer_object_constructed     (GObject                       *object);
static void            gtk_cifro_curve_renderer_object_finalize        (GObject                       *object);

static void            gtk_cifro_curve_renderer_area_changed           (GtkCifroCurveRendererPrivate  *priv,
                                                                        GtkCifroAreaSize              *size);
static void            gtk_cifro_curve_renderer_visible_changed        (GtkCifroCurveRendererPrivate  *priv,
                                                                        GtkCifroAreaSize              *size);
static void            gtk_cifro_curve_renderer_border_changed         (GtkCifroCurveRendererPrivate  *priv,
                                                                        GtkCifroAreaBorder            *border);

static gboolean        gtk_cifro_curve_renderer_check_visible_redraw   (GtkCifroCurveRendererPrivate  *priv);
static void            gtk_cifro_curve_renderer_visible_draw           (GtkCifroCurveRendererPrivate  *priv,
                                                                        cairo_t                       *cairo);

G_DEFINE_TYPE_WITH_PRIVATE (GtkCifroCurveRenderer, gtk_cifro_curve_renderer, G_TYPE_OBJECT);

static void
gtk_cifro_curve_renderer_class_init (GtkCifroCurveRendererClass *klass)
{

  GObjectClass *object_class = G_OBJECT_CLASS (klass);

  object_class->set_property = gtk_cifro_curve_renderer_set_property;

  object_class->constructed = gtk_cifro_curve_renderer_object_constructed;
  object_class->finalize = gtk_cifro_curve_renderer_object_finalize;
...
static void
gtk_cifro_curve_renderer_init (GtkCifroCurveRenderer *curve_renderer)
{
  curve_renderer->priv = gtk_cifro_curve_renderer_get_instance_private (curve_renderer);
}

static void
gtk_cifro_curve_renderer_set_property (GObject      *object,
                                       guint         prop_id,
                                       const GValue *value,
                                       GParamSpec   *pspec)
{
  GtkCifroCurveRenderer *curve_renderer = GTK_CIFRO_CURVE_RENDERER (object);
  GtkCifroCurveRendererPrivate *priv = curve_renderer->priv;
...
static void
gtk_cifro_curve_renderer_object_constructed (GObject *object)
{
  GtkCifroCurveRenderer *curve_renderer = GTK_CIFRO_CURVE_RENDERER (object);
  GtkCifroCurveRendererPrivate *priv = curve_renderer->priv;
  ...
  if (priv->area != NULL)
    {
      g_signal_connect_swapped (priv->area, "check-visible-redraw",
                                G_CALLBACK (gtk_cifro_curve_renderer_check_visible_redraw), priv);
      g_signal_connect_swapped (priv->area, "visible-draw",
                                G_CALLBACK (gtk_cifro_curve_renderer_visible_draw), priv);
    }
  ...
  if (priv->state != NULL)
    {
      g_signal_connect_swapped (priv->state, "area-changed",
                                G_CALLBACK( gtk_cifro_curve_renderer_area_changed ), priv);
      g_signal_connect_swapped (priv->state, "visible-changed",
                                G_CALLBACK( gtk_cifro_curve_renderer_visible_changed ), priv);
      g_signal_connect_swapped (priv->state, "border-changed",
                                G_CALLBACK( gtk_cifro_curve_renderer_border_changed ), priv);
    }
}

static void
gtk_cifro_curve_renderer_object_finalize (GObject *object)
{
  GtkCifroCurveRenderer *curve_renderer = GTK_CIFRO_CURVE_RENDERER (object);
  GtkCifroCurveRendererPrivate *priv = curve_renderer->priv;
  ...
  G_OBJECT_CLASS (gtk_cifro_curve_renderer_parent_class)->finalize (object);
}

/* Функция выбирает точку по указанным координатам. */
static void
gtk_cifro_curve_renderer_select_point (GtkCifroCurveRendererPrivate *priv,
                                       gint                          pointer_x,
                                       gint                          pointer_y)
{
  guint i;
  ...
  for (i = 0; i < priv->curve_points->len; i++)
    {
      ...
      if (x < -point_radius || x >= priv->area_width + point_radius || y < -point_radius
          || y >= priv->area_height + point_radius)
        continue;
...
static void
gtk_cifro_curve_renderer_area_changed (GtkCifroCurveRendererPrivate *priv,
                                       GtkCifroAreaSize             *size)
{
...
static void
gtk_cifro_curve_renderer_visible_changed (GtkCifroCurveRendererPrivate *priv,
                                          GtkCifroAreaSize             *size)
{
...
static void
gtk_cifro_curve_renderer_border_changed (GtkCifroCurveRendererPrivate *priv,
                                         GtkCifroAreaBorder           *border)
{
...
static gboolean
gtk_cifro_curve_renderer_check_visible_redraw (GtkCifroCurveRendererPrivate *priv)
{
...
static void
gtk_cifro_curve_renderer_visible_draw (GtkCifroCurveRendererPrivate *priv,
                                       cairo_t                      *cairo)
{
  ...
  for (i = 0; i < priv->curve_points->len; i++)
    {
      ...
      if (x < -point_radius || x >= priv->visible_width + point_radius || y < -point_radius
          || y >= priv->visible_height + point_radius)
        continue;

      cairo_arc (cairo, gtk_cifro_area_state_point_to_cairo (x), gtk_cifro_area_state_point_to_cairo (y),
                 point_radius / 4.0, 0.0, 2 * G_PI);
      cairo_fill (cairo);

      if (priv->selected_point != i)
        continue;

      cairo_arc (cairo, gtk_cifro_area_state_point_to_cairo (x), gtk_cifro_area_state_point_to_cairo (y), point_radius,
                 0.0, 2 * G_PI);
      cairo_stroke (cairo);
    }
...
GtkCifroCurveRenderer *
gtk_cifro_curve_renderer_new (GtkCifroArea     *area,
                                        GtkCifroCurveFunc curve_func,
                                        gpointer          curve_data)
{
  return g_object_new (GTK_TYPE_CIFRO_CURVE_RENDERER,
                       "area", area,
...
void
gtk_cifro_curve_renderer_clear_points (GtkCifroCurveRenderer *curve_renderer)
{
  g_return_if_fail (GTK_IS_CIFRO_CURVE_RENDERER (curve_renderer));

  g_array_set_size (curve_renderer->priv->curve_points, 0);

  curve_renderer->priv->visible_update = TRUE;
}

/* Функция добавляет одну контрольную точку к существующим. */
void
gtk_cifro_curve_renderer_add_point (GtkCifroCurveRenderer *curve_renderer,
                                    gdouble                x,
                                    gdouble                y)
{
  GtkCifroCurveRendererPrivate *priv;
...
void
gtk_cifro_curve_renderer_set_points (GtkCifroCurveRenderer *curve_renderer,
                                     GArray                *points)
{
  GtkCifroCurveRendererPrivate *priv;
...
GArray *
gtk_cifro_curve_renderer_get_points (GtkCifroCurveRenderer *curve_renderer)
{
  GArray *points;

  points = g_array_new (FALSE, FALSE, sizeof(GtkCifroCurvePoint));
  g_array_insert_vals (points, 0,
                       curve_renderer->priv->curve_points->data,
                       curve_renderer->priv->curve_points->len);
...
void
gtk_cifro_curve_renderer_set_curve_color (GtkCifroCurveRenderer *curve_renderer,
                                          gdouble                red,
                                          gdouble                green,
                                          gdouble                blue)
{
  curve_renderer->priv->curve_color = cairo_sdline_color (red, green, blue, 1.0);
...
void
gtk_cifro_curve_renderer_set_point_color (GtkCifroCurveRenderer *curve_renderer,
                                          gdouble                red,
                                          gdouble                green,
                                          gdouble                blue)
{
  curve_renderer->priv->point_color = cairo_sdline_color (red, green, blue, 1.0);
...
gboolean
gtk_cifro_curve_renderer_button_press_event (GtkWidget             *widget,
                                             GdkEventButton        *event,
                                             GtkCifroCurveRenderer *curve_renderer)
{
  GtkCifroCurveRendererPrivate *priv;

  g_return_val_if_fail (GTK_IS_CIFRO_CURVE_RENDERER (curve_renderer), FALSE);
...
gboolean
gtk_cifro_curve_renderer_button_release_event (GtkWidget             *widget,
                                               GdkEventButton        *event,
                                               GtkCifroCurveRenderer *curve_renderer)
{
  GtkCifroCurveRendererPrivate *priv;
...
gboolean
gtk_cifro_curve_renderer_motion_notify_event (GtkWidget             *widget,
                                              ...
                                              GtkCifroCurveRenderer *curve_renderer)
{
  ...
  point_radius = 0.4 * priv->border_top;
