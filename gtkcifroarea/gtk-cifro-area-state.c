...
typedef struct GtkCifroAreaStatePriv {           // Внутренние данные объекта.
...
} GtkCifroAreaStatePriv;


#define GTK_CIFRO_AREA_STATE_GET_PRIVATE( obj ) ( G_TYPE_INSTANCE_GET_PRIVATE( ( obj ), GTK_TYPE_CIFRO_AREA_STATE, GtkCifroAreaStatePriv ) )


static void gtk_cifro_area_state_object_finalize( GObject *state );

static guint gtk_cifro_area_state_signals[ SIGNAL_LAST ] = { 0 };


G_DEFINE_TYPE( GtkCifroAreaState, gtk_cifro_area_state, G_TYPE_OBJECT );


static void gtk_cifro_area_state_class_init( GtkCifroAreaStateClass *klass )
{

  GObjectClass *this_class = G_OBJECT_CLASS( klass );

  this_class->finalize = gtk_cifro_area_state_object_finalize;

  gtk_cifro_area_state_signals[ SIGNAL_AREA_CHANGED ] =
    g_signal_new( "area-changed", GTK_TYPE_CIFRO_AREA_STATE, G_SIGNAL_RUN_LAST, 0, NULL, NULL,
                  g_cclosure_marshal_VOID__POINTER, G_TYPE_NONE, 1, G_TYPE_POINTER );

  gtk_cifro_area_state_signals[ SIGNAL_VISIBLE_CHANGED ] =
    g_signal_new( "visible-changed", GTK_TYPE_CIFRO_AREA_STATE, G_SIGNAL_RUN_LAST, 0, NULL, NULL,
                  g_cclosure_marshal_VOID__POINTER, G_TYPE_NONE, 1, G_TYPE_POINTER );

  gtk_cifro_area_state_signals[ SIGNAL_BORDER_CHANGED ] =
    g_signal_new( "border-changed", GTK_TYPE_CIFRO_AREA_STATE, G_SIGNAL_RUN_LAST, 0, NULL, NULL,
                  g_cclosure_marshal_VOID__POINTER, G_TYPE_NONE, 1, G_TYPE_POINTER );

  gtk_cifro_area_state_signals[ SIGNAL_SWAP_CHANGED ] =
    g_signal_new( "swap-changed", GTK_TYPE_CIFRO_AREA_STATE, G_SIGNAL_RUN_LAST, 0, NULL, NULL,
                  g_cclosure_marshal_VOID__POINTER, G_TYPE_NONE, 1, G_TYPE_POINTER );

  gtk_cifro_area_state_signals[ SIGNAL_ANGLE_CHANGED ] =
    g_signal_new( "angle-changed", GTK_TYPE_CIFRO_AREA_STATE, G_SIGNAL_RUN_LAST, 0, NULL, NULL,
                  g_cclosure_marshal_VOID__DOUBLE, G_TYPE_NONE, 1, G_TYPE_DOUBLE );

  gtk_cifro_area_state_signals[ SIGNAL_VIEW_LIMITS_CHANGED ] =
    g_signal_new( "view-limits-changed", GTK_TYPE_CIFRO_AREA_STATE, G_SIGNAL_RUN_LAST, 0, NULL, NULL,
                  g_cclosure_marshal_VOID__POINTER, G_TYPE_NONE, 1, G_TYPE_POINTER );

  gtk_cifro_area_state_signals[ SIGNAL_VIEW_CHANGED ] =
    g_signal_new( "view-changed", GTK_TYPE_CIFRO_AREA_STATE, G_SIGNAL_RUN_LAST, 0, NULL, NULL,
                  g_cclosure_marshal_VOID__POINTER, G_TYPE_NONE, 1, G_TYPE_POINTER );

  g_type_class_add_private( klass, sizeof( GtkCifroAreaStatePriv ) );

}


static void gtk_cifro_area_state_init( GtkCifroAreaState *state )
{

  GtkCifroAreaStatePriv *priv = GTK_CIFRO_AREA_STATE_GET_PRIVATE( state );
...
static void gtk_cifro_area_state_object_finalize( GObject *state )
{
  ...
  G_OBJECT_CLASS( gtk_cifro_area_state_parent_class )->finalize( state );
...
void gtk_cifro_area_state_set_area_size( GtkCifroAreaState *state, gint width, gint height )
{
  ...
  if( emit ) g_signal_emit( state, gtk_cifro_area_state_signals[ SIGNAL_AREA_CHANGED ], 0, &area_size );
...
void gtk_cifro_area_state_get_area_size( GtkCifroAreaState *state, gint *width, gint *height )
{
  ...
  if( width ) *width = priv->area_width;
  if( height ) *height = priv->area_height;
...
void gtk_cifro_area_state_set_visible_size( GtkCifroAreaState *state, gint width, gint height )
{
  ...
  if( emit ) g_signal_emit( state, gtk_cifro_area_state_signals[ SIGNAL_VISIBLE_CHANGED ], 0, &visible_size );
...
void gtk_cifro_area_state_get_visible_size( GtkCifroAreaState *state, gint *width, gint *height )
{
  ...
  if( width ) *width = priv->visible_width;
  if( height ) *height = priv->visible_height;
...
void gtk_cifro_area_state_set_border( GtkCifroAreaState *state, gint left, gint right, gint top, gint bottom )
{
  ...
  if( priv->border_left != left || priv->border_right != right|| priv->border_top != top || priv->border_bottom != bottom )
    {
  ...
  if( emit ) g_signal_emit( state, gtk_cifro_area_state_signals[ SIGNAL_BORDER_CHANGED ], 0, &border );
...
void gtk_cifro_area_state_set_swap( GtkCifroAreaState *state, gboolean swap_x, gboolean swap_y )
{
  ...
  if( emit ) g_signal_emit( state, gtk_cifro_area_state_signals[ SIGNAL_SWAP_CHANGED ], 0, &swap );
...
void gtk_cifro_area_state_set_angle( GtkCifroAreaState *state, gdouble angle )
{
  ...
  if( emit ) g_signal_emit( state, gtk_cifro_area_state_signals[ SIGNAL_ANGLE_CHANGED ], 0, angle );
...
void gtk_cifro_area_state_set_view_limits( GtkCifroAreaState *state, gdouble min_x, gdouble max_x, gdouble min_y, gdouble max_y )
{
  ...
  if( priv->min_x != min_x || priv->max_x != max_x || priv->min_y != min_y || priv->max_y != max_y )
    {
  ...
  if( emit ) g_signal_emit( state, gtk_cifro_area_state_signals[ SIGNAL_VIEW_LIMITS_CHANGED ], 0, &view );
...
void gtk_cifro_area_state_set_view( GtkCifroAreaState *state, gdouble from_x, gdouble to_x, gdouble from_y, gdouble to_y )
{
  ...
  if( priv->from_x != from_x || priv->to_x != to_x || priv->from_y != from_y || priv->to_y != to_y )
    {
  ...
  if( emit ) g_signal_emit( state, gtk_cifro_area_state_signals[ SIGNAL_VIEW_CHANGED ], 0, &view );
...
void gtk_cifro_area_state_get_scale( GtkCifroAreaState *state, gdouble *scale_x, gdouble *scale_y )
{
  ...
  if( scale_x ) *scale_x = priv->scale_x;
  if( scale_y ) *scale_y = priv->scale_y;
...
void gtk_cifro_area_state_value_to_point( GtkCifroAreaState *state, gdouble *x, gdouble *y, gdouble x_val, gdouble y_val )
{
  ...
  // Если включено зеркальное отражение по осям - переносим координаты.
  if( priv->swap_x ) x_val = -x_val;
  if( priv->swap_y ) y_val = -y_val;
...
void gtk_cifro_area_state_visible_point_to_value( GtkCifroAreaState *state, gdouble x, gdouble y, gdouble *x_val, gdouble *y_val )
{
  ...
  if( x_val ) *x_val = priv->from_x + x * priv->scale_x;
  if( y_val ) *y_val = priv->to_y - y * priv->scale_y;
...
void gtk_cifro_area_state_visible_value_to_point( GtkCifroAreaState *state, gdouble *x, gdouble *y, gdouble x_val, gdouble y_val )
{
  ...
  if( x ) *x = ( x_val - priv->from_x ) / priv->scale_x;
  if( y ) *y = ( priv->to_y - y_val ) / priv->scale_y;
...
void gtk_cifro_area_state_get_axis_step( gdouble scale, gdouble step_width, gdouble *from, gdouble *step, gint *range, gint *power )
{
  ...
  *from = from_ret;
  if( step ) *step = step_ret;
  if( range ) *range = range_ret;
  if( power ) *power = power_ret;
