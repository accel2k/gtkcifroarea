...
*/
static gint _clipEncode( gint x, gint y, gint left, gint top, gint right, gint bottom )
{
  gint code = 0;
...
*/
static gint _clipLine( cairo_sdline_surface *surface, gint *x1, gint *y1, gint *x2, gint *y2 )
{
...
void cairo_sdline_set_cairo_color( cairo_t *cairo, guint32 color )
{
...
guint32 cairo_sdline_color( double red, double green, double blue, double alpha )
{
...
void cairo_sdline_clear_color( cairo_sdline_surface *surface, guint32 color )
{
  ...
  for( i = 0, shift = 0; i < surface->width; i++, shift += PIXEL_SIZE )
    *(guint32*)( surface->data + shift ) = color;
...
void cairo_sdline_h( cairo_sdline_surface *surface, gint x1, gint x2, gint y1, guint32 color )
{
...
void cairo_sdline_v( cairo_sdline_surface *surface, gint x1, gint y1, gint y2, guint32 color )
{
...
void cairo_sdline( cairo_sdline_surface *surface, gint x1, gint y1, gint x2, gint y2, guint32 color )
{
...
void cairo_sdline_bar( cairo_sdline_surface *surface, gint x1, gint y1, gint x2, gint y2, guint32 color )
{
...
void cairo_sdline_dot( cairo_sdline_surface *surface, gint x, gint y, guint32 color )
{

  if( x < 0 || y < 0 ) return;
  if( x >= surface->width || y >= surface->height ) return;

  gint shift = ( y * surface->stride ) + PIXEL_SIZE * x;
  *(guint32*)( surface->data + shift ) = color;
