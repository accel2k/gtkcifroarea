...
enum { SIGNAL_CHECK_VISIBLE_REDRAW,
...
typedef struct GtkCifroAreaPriv {
...
#define GTK_CIFRO_AREA_GET_PRIVATE( obj ) ( G_TYPE_INSTANCE_GET_PRIVATE( ( obj ), GTK_TYPE_CIFRO_AREA, GtkCifroAreaPriv ) )


static GObject* gtk_cifro_area_object_constructor( GType type, guint n_construct_properties, GObjectConstructParam *construct_properties );
static void gtk_cifro_area_object_finalize( GObject *carea );

static gint gtk_cifro_area_get_visible_width( gdouble width, gdouble height, gdouble angle );
static gint gtk_cifro_area_get_visible_height( gdouble width, gdouble height, gdouble angle );
static gboolean gtk_cifro_area_fix_scale_aspect( gdouble scale_aspect, gdouble *min_scale_x, gdouble *max_scale_x, gdouble *min_scale_y, gdouble *max_scale_y );
static void gtk_cifro_area_update_visible( GtkCifroAreaPriv *priv );

static gboolean gtk_cifro_area_key_press( GtkWidget *widget, GdkEventKey *event, GtkCifroAreaPriv *priv );
static gboolean gtk_cifro_area_button_press_release( GtkWidget *widget, GdkEventButton *event, GtkCifroAreaPriv *priv );
static gboolean gtk_cifro_area_motion( GtkWidget *widget, GdkEventMotion *event, GtkCifroAreaPriv *priv );
static gboolean gtk_cifro_area_scroll( GtkWidget *widget, GdkEventScroll *event, GtkCifroAreaPriv *priv );

static gboolean gtk_cifro_area_configure( GtkWidget *widget, GdkEventConfigure *event, GtkCifroAreaPriv *priv );
static gboolean gtk_cifro_area_draw( GtkWidget *widget, cairo_t *cairo, GtkCifroAreaPriv *priv );
#ifdef CIFRO_AREA_WITH_GTK2
static gboolean gtk_cifro_area_expose( GtkWidget *widget, GdkEventExpose *event, GtkCifroAreaPriv *priv );
#endif

static guint gtk_cifro_area_signals[ SIGNAL_LAST ] = { 0 };


G_DEFINE_TYPE( GtkCifroArea, gtk_cifro_area, GTK_TYPE_DRAWING_AREA )


static void gtk_cifro_area_init( GtkCifroArea *carea ){ ; }


static void gtk_cifro_area_class_init( GtkCifroAreaClass *klass )
{

  GObjectClass *this_class = G_OBJECT_CLASS( klass );

  this_class->constructor = gtk_cifro_area_object_constructor;
  this_class->finalize = gtk_cifro_area_object_finalize;

  gtk_cifro_area_signals[ SIGNAL_CHECK_VISIBLE_REDRAW ] =
    g_signal_new( "check-visible-redraw", GTK_TYPE_CIFRO_AREA, G_SIGNAL_RUN_LAST, 0, g_signal_accumulator_true_handled, NULL,
                g_cclosure_user_marshal_BOOLEAN__VOID, G_TYPE_BOOLEAN, 0 );

  gtk_cifro_area_signals[ SIGNAL_CHECK_AREA_REDRAW ] =
    g_signal_new( "check-area-redraw", GTK_TYPE_CIFRO_AREA, G_SIGNAL_RUN_LAST, 0, g_signal_accumulator_true_handled, NULL,
                g_cclosure_user_marshal_BOOLEAN__VOID, G_TYPE_BOOLEAN, 0 );

  gtk_cifro_area_signals[ SIGNAL_VISIBLE_DRAW ] =
    g_signal_new( "visible-draw", GTK_TYPE_CIFRO_AREA, G_SIGNAL_RUN_LAST, 0, NULL, NULL,
                g_cclosure_marshal_VOID__POINTER, G_TYPE_NONE, 1, G_TYPE_POINTER );

  gtk_cifro_area_signals[ SIGNAL_AREA_DRAW ] =
    g_signal_new( "area-draw", GTK_TYPE_CIFRO_AREA, G_SIGNAL_RUN_LAST, 0, NULL, NULL,
                g_cclosure_marshal_VOID__POINTER, G_TYPE_NONE, 1, G_TYPE_POINTER );

  g_type_class_add_private( klass, sizeof( GtkCifroAreaPriv ) );

}


static GObject* gtk_cifro_area_object_constructor( GType g_type, guint n_construct_properties, GObjectConstructParam *construct_properties )
{

  GObject *carea = G_OBJECT_CLASS( gtk_cifro_area_parent_class )->constructor( g_type, n_construct_properties, construct_properties );
  GtkCifroAreaPriv *priv = GTK_CIFRO_AREA_GET_PRIVATE( carea );
  ...
  g_signal_connect( carea, "configure-event", G_CALLBACK( gtk_cifro_area_configure ), priv );
  g_signal_connect_after( carea , "key-press-event", G_CALLBACK( gtk_cifro_area_key_press ), priv );
  g_signal_connect_after( carea , "button-press-event", G_CALLBACK( gtk_cifro_area_button_press_release ), priv );
  g_signal_connect_after( carea , "button-release-event", G_CALLBACK( gtk_cifro_area_button_press_release ), priv );
  g_signal_connect_after( carea , "motion-notify-event", G_CALLBACK( gtk_cifro_area_motion ), priv );
  g_signal_connect_after( carea , "scroll-event", G_CALLBACK( gtk_cifro_area_scroll ), priv );

#ifdef CIFRO_AREA_WITH_GTK2
  g_signal_connect( carea , "expose-event", G_CALLBACK( gtk_cifro_area_expose ), priv );
#else
  g_signal_connect( carea , "draw", G_CALLBACK( gtk_cifro_area_draw ), priv );
#endif
...
static void gtk_cifro_area_object_finalize( GObject *carea )
{
  ...
  G_OBJECT_CLASS( gtk_cifro_area_parent_class )->finalize( G_OBJECT( carea ) );
...
static void gtk_cifro_area_update_visible( GtkCifroAreaPriv *priv )
{
  ...
  if( ( priv->visible_width != ivisible_width ) || ( priv->visible_height != ivisible_height ) )
      ...
      if( ivisible_width > priv->visible_width || ivisible_height > priv->visible_height )
        ...
        if( priv->visible_cairo ) cairo_destroy( priv->visible_cairo );
...
static gboolean gtk_cifro_area_scroll( GtkWidget *widget, GdkEventScroll *event, GtkCifroAreaPriv *priv )
{
  ...
  if( ( event->y < priv->clip_y ) || ( event->y > priv->clip_x + priv->clip_height ) ) return TRUE;
...
static gboolean gtk_cifro_area_configure( GtkWidget *widget, GdkEventConfigure *event, GtkCifroAreaPriv *priv )
{
  ...
  if( priv->widget_width == widget_width && priv->widget_height == widget_height ) return FALSE;
...
static gboolean gtk_cifro_area_expose( GtkWidget *widget, GdkEventExpose *event, GtkCifroAreaPriv *priv )
{
  ...
  gboolean status = gtk_cifro_area_draw( widget, cairo, priv );
  cairo_destroy( cairo );
  return status;
...
// Функция возвращает указатель на объект IcaStateRenderer, если такой зарегистрирован в CifroArea.
...
void gtk_cifro_area_update( GtkCifroArea *carea )
{
  ...
  if( redraw ) priv->update_visible = TRUE;
  else // ... или проверяем необходимость перерисовки области виджета.
...
gboolean gtk_cifro_area_set_scale_aspect( GtkCifroArea *carea, gdouble scale_aspect )
{
  ...
  g_return_val_if_fail( gtk_cifro_area_fix_scale_aspect( scale_aspect, &priv->min_scale_x, &priv->max_scale_x, &priv->min_scale_y, &priv->max_scale_y ), FALSE );
...
gboolean gtk_cifro_area_set_scale_limits( GtkCifroArea *carea, gdouble min_scale_x, gdouble max_scale_x, gdouble min_scale_y, gdouble max_scale_y )
{
  ...
  g_return_val_if_fail( gtk_cifro_area_fix_scale_aspect( priv->scale_aspect, &min_scale_x, &max_scale_x, &min_scale_y, &max_scale_y ), FALSE );
...
gboolean gtk_cifro_area_set_view( GtkCifroArea *carea, gdouble from_x, gdouble to_x, gdouble from_y, gdouble to_y )
{
  ...
  if( priv->num_scales == 0 )
    ...
    if( priv->widget_width && priv->widget_height )
      {
      opt_scale_x = ( priv->to_x - priv->from_x ) / gtk_cifro_area_get_visible_width( priv->widget_width, priv->widget_height, priv->angle );
      opt_scale_y = ( priv->to_y - priv->from_y ) / gtk_cifro_area_get_visible_height( priv->widget_width, priv->widget_height, priv->angle );
      }
    else
      {
      opt_scale_x = 1.0;
