...
struct _GtkCifroScopeRendererPrivate
{
  GtkCifroArea                *area;                   /* Указатель на объект GtkCifroArea. */
...
static void            gtk_cifro_scope_renderer_set_property           (GObject                       *object,
                                                                        guint                          prop_id,
                                                                        const GValue                  *value,
                                                                        GParamSpec                    *pspec);
static void            gtk_cifro_scope_renderer_object_constructed     (GObject                       *object);
static void            gtk_cifro_scope_renderer_object_finalize        (GObject                       *object);

static void            gtk_cifro_scope_renderer_free_channel           (GtkCifroScopeChannel          *channel);

static void            gtk_cifro_scope_renderer_area_changed           (GtkCifroScopeRendererPrivate  *priv,
                                                                        GtkCifroAreaSize              *size);
static void            gtk_cifro_scope_renderer_visible_changed        (GtkCifroScopeRendererPrivate  *priv,
                                                                        GtkCifroAreaSize              *size);
static void            gtk_cifro_scope_renderer_border_changed         (GtkCifroScopeRendererPrivate  *priv,
                                                                        GtkCifroAreaBorder            *border);
static void            gtk_cifro_scope_renderer_swap_changed           (GtkCifroScopeRendererPrivate  *priv,
                                                                        GtkCifroAreaSwap              *swap);
static void            gtk_cifro_scope_renderer_angle_changed          (GtkCifroScopeRendererPrivate  *priv,
                                                                        gdouble                        angle);
static void            gtk_cifro_scope_renderer_view_limits_changed    (GtkCifroScopeRendererPrivate  *priv,
                                                                        GtkCifroAreaView              *view);
static void            gtk_cifro_scope_renderer_view_changed           (GtkCifroScopeRendererPrivate  *priv,
                                                                        GtkCifroAreaView              *view);

static void            gtk_cifro_scope_renderer_draw_hruler            (cairo_t                       *cairo,
                                                                        GtkCifroScopeRendererPrivate  *priv);
static void            gtk_cifro_scope_renderer_draw_vruler            (cairo_t                       *cairo,
                                                                        GtkCifroScopeRendererPrivate  *priv);
static void            gtk_cifro_scope_renderer_draw_x_pos             (cairo_t                       *cairo,
                                                                        GtkCifroScopeRendererPrivate  *priv);
static void            gtk_cifro_scope_renderer_draw_y_pos             (cairo_t                       *cairo,
                                                                        GtkCifroScopeRendererPrivate  *priv);
static void            gtk_cifro_scope_renderer_draw_info              (cairo_t                       *cairo,
                                                                        GtkCifroScopeRendererPrivate  *priv);

static void            gtk_cifro_scope_renderer_draw_axis              (cairo_sdline_surface          *surface,
                                                                        GtkCifroScopeRendererPrivate  *priv);
static void            gtk_cifro_scope_renderer_draw_lined_data        (cairo_sdline_surface          *surface,
                                                                        GtkCifroScopeRendererPrivate  *priv,
                                                                        gpointer                       channel_id);
static void            gtk_cifro_scope_renderer_draw_dotted_data       (cairo_sdline_surface          *surface,
                                                                        GtkCifroScopeRendererPrivate  *priv,
                                                                        gpointer                       channel_id);

static gboolean        gtk_cifro_scope_renderer_check_area_redraw      (GtkCifroScopeRendererPrivate  *priv);
static gboolean        gtk_cifro_scope_renderer_check_visible_redraw   (GtkCifroScopeRendererPrivate  *priv);
static void            gtk_cifro_scope_renderer_area_draw              (GtkCifroScopeRendererPrivate  *priv,
                                                                        cairo_t                       *cairo);
static void            gtk_cifro_scope_renderer_visible_draw           (GtkCifroScopeRendererPrivate  *priv,
                                                                        cairo_t                       *cairo);

G_DEFINE_TYPE_WITH_PRIVATE (GtkCifroScopeRenderer, gtk_cifro_scope_renderer, G_TYPE_OBJECT);

static void gtk_cifro_scope_renderer_class_init (GtkCifroScopeRendererClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);

  object_class->set_property = gtk_cifro_scope_renderer_set_property;

  object_class->constructed = gtk_cifro_scope_renderer_object_constructed;
  object_class->finalize = gtk_cifro_scope_renderer_object_finalize;

  g_object_class_install_property (object_class, PROP_AREA,
    g_param_spec_object ("area", "Area", "GtkCifroArea object", GTK_TYPE_CIFRO_AREA,
                         G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY));
}

static void
gtk_cifro_scope_renderer_init (GtkCifroScopeRenderer *scope_renderer)
{
  scope_renderer->priv = gtk_cifro_scope_renderer_get_instance_private (scope_renderer);
}

static void
gtk_cifro_scope_renderer_set_property (GObject      *object,
                                       guint         prop_id,
                                       const GValue *value,
                                       GParamSpec   *pspec)
{
  GtkCifroScopeRenderer *scope_renderer = GTK_CIFRO_SCOPE_RENDERER (object);
  GtkCifroScopeRendererPrivate *priv = scope_renderer->priv;
...
static void
gtk_cifro_scope_renderer_object_constructed (GObject *object)
{
  GtkCifroScopeRenderer *scope_renderer = GTK_CIFRO_SCOPE_RENDERER (object);
  GtkCifroScopeRendererPrivate *priv = scope_renderer->priv;
  ...
  if (priv->area != NULL)
    priv->state = gtk_cifro_area_get_state (priv->area);
  ...
  priv->channels = g_hash_table_new_full (g_direct_hash, g_direct_equal, NULL,
                                          (GDestroyNotify) gtk_cifro_scope_renderer_free_channel);
  ...
  if (priv->area != NULL)
    {
      g_signal_connect_swapped (priv->area, "check-area-redraw",
                                G_CALLBACK (gtk_cifro_scope_renderer_check_area_redraw), priv);
      g_signal_connect_swapped (priv->area, "check-visible-redraw",
                                G_CALLBACK (gtk_cifro_scope_renderer_check_visible_redraw), priv);
      g_signal_connect_swapped (priv->area, "area-draw",
                                G_CALLBACK (gtk_cifro_scope_renderer_area_draw), priv);
      g_signal_connect_swapped (priv->area, "visible-draw",
                                G_CALLBACK (gtk_cifro_scope_renderer_visible_draw), priv);
    }

  /* Обработчики сигналов состояния GtkCifroArea. */
  if (priv->state != NULL)
    {
      g_signal_connect_swapped (priv->state, "area-changed",
                                G_CALLBACK (gtk_cifro_scope_renderer_area_changed), priv);
      g_signal_connect_swapped (priv->state, "visible-changed",
                                G_CALLBACK (gtk_cifro_scope_renderer_visible_changed), priv);
      g_signal_connect_swapped (priv->state, "border-changed",
                                G_CALLBACK (gtk_cifro_scope_renderer_border_changed), priv);
      g_signal_connect_swapped (priv->state, "swap-changed",
                                G_CALLBACK (gtk_cifro_scope_renderer_swap_changed), priv);
      g_signal_connect_swapped (priv->state, "angle-changed",
                                G_CALLBACK (gtk_cifro_scope_renderer_angle_changed), priv);
      g_signal_connect_swapped (priv->state, "view-limits-changed",
                                G_CALLBACK (gtk_cifro_scope_renderer_view_limits_changed), priv);
      g_signal_connect_swapped (priv->state, "view-changed",
                                G_CALLBACK (gtk_cifro_scope_renderer_view_changed), priv);
    }
}

static void
gtk_cifro_scope_renderer_object_finalize (GObject *object)
{
  GtkCifroScopeRenderer *scope_renderer = GTK_CIFRO_SCOPE_RENDERER (object);
  GtkCifroScopeRendererPrivate *priv = scope_renderer->priv;
  ...
  G_OBJECT_CLASS (gtk_cifro_scope_renderer_parent_class)->finalize (object);
}

/* Функция удаляет структуру с данными канала. */
static void
gtk_cifro_scope_renderer_free_channel (GtkCifroScopeChannel *channel)
{
  if (channel == NULL)
    return;
...
static void
gtk_cifro_scope_renderer_area_changed (GtkCifroScopeRendererPrivate *priv,
                                       GtkCifroAreaSize             *size)
{
...
static void
gtk_cifro_scope_renderer_visible_changed (GtkCifroScopeRendererPrivate *priv,
                                          GtkCifroAreaSize             *size)
{
...
static void
gtk_cifro_scope_renderer_border_changed (GtkCifroScopeRendererPrivate *priv,
                                         GtkCifroAreaBorder           *border)
{
...
static void
gtk_cifro_scope_renderer_swap_changed (GtkCifroScopeRendererPrivate *priv,
                                       GtkCifroAreaSwap             *swap)
{
...
static void
gtk_cifro_scope_renderer_angle_changed (GtkCifroScopeRendererPrivate *priv,
                                        gdouble                       angle)
{
  priv->angle = angle;
...
static void
gtk_cifro_scope_renderer_view_limits_changed (GtkCifroScopeRendererPrivate *priv,
                                              GtkCifroAreaView             *view)
{
  priv->min_x = view->x1;
  priv->max_x = view->x2;
...
static void
gtk_cifro_scope_renderer_view_changed (GtkCifroScopeRendererPrivate *priv,
                                       GtkCifroAreaView             *view)
{
  priv->from_x = view->x1;
  priv->to_x = view->x2;
...
static void
gtk_cifro_scope_renderer_draw_hruler (cairo_t                      *cairo,
                                      GtkCifroScopeRendererPrivate *priv)
{
  ...
  if (priv->state == NULL || priv->font == NULL)
    return;
...
static void
gtk_cifro_scope_renderer_draw_vruler (cairo_t                      *cairo,
                                      GtkCifroScopeRendererPrivate *priv)
{
  GtkCifroAreaState *state = priv->state;
  PangoLayout *font = priv->font;
...
static void
gtk_cifro_scope_renderer_draw_x_pos (cairo_t                      *cairo,
                                     GtkCifroScopeRendererPrivate *priv)
{
  gint area_width = priv->area_width;
  gint area_height = priv->area_height;
...
static void
gtk_cifro_scope_renderer_draw_y_pos (cairo_t                      *cairo,
                                     GtkCifroScopeRendererPrivate *priv)
{
  gint area_width = priv->area_width;
  gint area_height = priv->area_height;
...
static void
gtk_cifro_scope_renderer_draw_info (cairo_t                      *cairo,
                                    GtkCifroScopeRendererPrivate *priv)
{
...
static void
gtk_cifro_scope_renderer_draw_axis (cairo_sdline_surface         *surface,
                                    GtkCifroScopeRendererPrivate *priv)
{
  GtkCifroAreaState *state = priv->state;
...
static void
gtk_cifro_scope_renderer_draw_lined_data (cairo_sdline_surface         *surface,
                                          GtkCifroScopeRendererPrivate *priv,
                                          gpointer                      channel_id)
{
...
static void
gtk_cifro_scope_renderer_draw_dotted_data (cairo_sdline_surface         *surface,
                                           GtkCifroScopeRendererPrivate *priv,
                                           gpointer                      channel_id)
{
...
static gboolean
gtk_cifro_scope_renderer_check_area_redraw (GtkCifroScopeRendererPrivate *priv)
{
  if (!priv->area_update && !priv->visible_update)
    return FALSE;
...
static gboolean
gtk_cifro_scope_renderer_check_visible_redraw (GtkCifroScopeRendererPrivate *priv)
{
  if (!priv->visible_update)
    return FALSE;
...
static void
gtk_cifro_scope_renderer_area_draw (GtkCifroScopeRendererPrivate *priv,
                                    cairo_t                      *cairo)
{
  ...
  if (priv->show_info)
    gtk_cifro_scope_renderer_draw_info (cairo, priv);
}

/* Функция рисования видимой области (осциллограмм). */
static void
gtk_cifro_scope_renderer_visible_draw (GtkCifroScopeRendererPrivate *priv,
                                       cairo_t                      *cairo)
{
...
GtkCifroScopeRenderer *
gtk_cifro_scope_renderer_new (GtkCifroArea *area)
{
...
void
gtk_cifro_scope_renderer_set_font (GtkCifroScopeRenderer *scope_renderer,
                                   PangoLayout           *font)
{
  GtkCifroScopeRendererPrivate *priv;

  g_return_if_fail (GTK_IS_CIFRO_SCOPE_RENDERER (scope_renderer));

  priv = scope_renderer->priv;
...
void
gtk_cifro_scope_renderer_set_fg_color (GtkCifroScopeRenderer *scope_renderer,
                                       gdouble                red,
                                       gdouble                green,
                                       gdouble                blue,
                                       gdouble                alpha)
...
void
gtk_cifro_scope_renderer_set_pointer (GtkCifroScopeRenderer *scope_renderer,
                                      gint                   pointer_x,
                                      gint                   pointer_y)
{
  g_return_if_fail (GTK_IS_CIFRO_SCOPE_RENDERER (scope_renderer));

  scope_renderer->priv->pointer_x = pointer_x;
  scope_renderer->priv->pointer_y = pointer_y;

  scope_renderer->priv->area_update = TRUE;
}

/* Функция включает или выключает отображения блока с информацией о значениях под курсором. */
void
gtk_cifro_scope_renderer_set_info_show (GtkCifroScopeRenderer *scope_renderer,
                                        gboolean               show)
{
  g_return_if_fail (GTK_IS_CIFRO_SCOPE_RENDERER (scope_renderer));

  scope_renderer->priv->show_info = show;
...
void
gtk_cifro_scope_renderer_set_axis_name (GtkCifroScopeRenderer *scope_renderer,
...
gpointer
gtk_cifro_scope_renderer_add_channel (GtkCifroScopeRenderer *scope_renderer)
{
  GtkCifroScopeRendererPrivate *priv;
  GtkCifroScopeChannel* channel;
  gpointer channel_id;

  g_return_val_if_fail (GTK_IS_CIFRO_SCOPE_RENDERER (scope_renderer), NULL);
...
void
gtk_cifro_scope_renderer_remove_channel (GtkCifroScopeRenderer *scope_renderer,
                                         gpointer               channel_id)
{
  g_return_if_fail (GTK_IS_CIFRO_SCOPE_RENDERER (scope_renderer));

  g_hash_table_remove (scope_renderer->priv->channels, channel_id);

  scope_renderer->priv->visible_update = TRUE;
}

/* Функция устанавливает имя канала. */
void
gtk_cifro_scope_renderer_set_channel_name (GtkCifroScopeRenderer *scope_renderer,
                                           gpointer               channel_id,
                                           const gchar           *axis_name)
{
  ...
  g_hash_table_iter_init (&channels_iter, scope_renderer->priv->channels);
...
void
gtk_cifro_scope_renderer_set_channel_time_param (GtkCifroScopeRenderer *scope_renderer,
                                                 gpointer               channel_id,
                                                 gfloat                 time_shift,
                                                 gfloat                 time_step)
{
...
void
gtk_cifro_scope_renderer_set_channel_value_param (GtkCifroScopeRenderer *scope_renderer,
                                                  gpointer               channel_id,
                                                  gfloat                 value_shift,
                                                  gfloat                 value_scale)
{
...
void
gtk_cifro_scope_renderer_set_channel_draw_type (GtkCifroScopeRenderer *scope_renderer,
                                                gpointer               channel_id,
...
void
gtk_cifro_scope_renderer_set_channel_color (GtkCifroScopeRenderer *scope_renderer,
                                            gpointer               channel_id,
                                            gdouble                red,
                                            gdouble                green,
                                            gdouble                blue)
...
void
gtk_cifro_scope_renderer_set_channel_data (GtkCifroScopeRenderer *scope_renderer,
                                           gpointer               channel_id,
                                           gint                   num,
                                           gfloat                *values)
{
  ...
  channel = g_hash_table_lookup (scope_renderer->priv->channels, channel_id);
  if (!channel)
    return;
...
void
gtk_cifro_scope_renderer_set_channel_show (GtkCifroScopeRenderer *scope_renderer,
                                           gpointer               channel_id,
                                           gboolean               show)
{
  GHashTableIter channels_iter;
...
void
gtk_cifro_scope_renderer_update (GtkCifroScopeRenderer *scope_renderer)
{
  g_return_if_fail (GTK_IS_CIFRO_SCOPE_RENDERER (scope_renderer));
