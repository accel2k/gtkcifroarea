...
static gchar *channel_colors[ MAX_N_CHANNELS + 2 ] = {
  ...
  "red", "green", "blue", "orange", "magenta", "lime", "cyan", "goldenrod", "purple", NULL
...
void destroy_callback( GtkWidget *widget, gpointer user_data )
{

  GArray *points = gtk_cifro_curve_get_points( GTK_CIFRO_CURVE( user_data ) );
  GtkCifroCurvePoint *point;
  gint i;
...
gboolean update_data( gpointer user_data )
{
  ...
  for( k = 0; k < n_channels; k++ )
    if( data[k] == NULL ) data[k] = g_new0( gfloat, n_points );
  ...
  for( k = 0; k < n_channels - 1; k++ )
    ...
    for( i = 0; i < n_points; i++ )
      ...
      gdouble dt = ( (gdouble)k / (gdouble)( n_channels - 1 ) ) + ( (gdouble)( j * ( n_channels - k - 1 ) ) / ( 4.0 * max_time ) );
    ...
    if( k == 0 )
      ...
      for( i = n_points / 3; i < 2 * n_points / 3; i++ )
        if( i % ( n_points / 32 ) > ( n_points / 64 ) ) data[0][i] = NAN;
...
int main( int argc, char **argv )
{
  ...
  if( n_channels > MAX_N_CHANNELS ) n_channels = MAX_N_CHANNELS;

  // Ограничения по числу точек в канале.
  if( n_points < 32 ) n_points = 32;
  if( n_points > 100000 ) n_points = 100000;
  ...
  gtk_cifro_area_set_view_limits( GTK_CIFRO_AREA( area ), -( max_time / 100.0 ), max_time + max_time / 100.0, -( max_range + max_range / 10.0 ), max_range + max_range / 10.0 );
...
//  gtk_cifro_scope_set_info_show( GTK_CIFRO_SCOPE( area ), TRUE );
  ...
  for( i = 0; i < n_channels - 1; i++ )
    gtk_cifro_scope_set_channel_value_param( GTK_CIFRO_SCOPE( area ), channels[i], 0.0, 0.25 + 0.75 * ( (gdouble)( i + 1 ) / (gdouble)( n_channels - 1 ) ) );
  ...
  g_timeout_add( 20, update_data, area );
